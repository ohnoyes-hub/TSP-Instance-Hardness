...
class TSPBuilder:
    ...
    def _generated_symmetric_tsp(self):
        """
        Generate a symmetric TSP instance with the upper triangle of the matrix filled with random values.
        Lower triangle is mirror of upper triangle.
        The diagonal is set to infinity to indicate no self-loops.
        """
        if self.distribution == 'uniform':
            upper_triangle = np.triu(np.random.randint(0, self.control + 1, size=(self.city_size, self.city_size)))
        else: # lognormal
